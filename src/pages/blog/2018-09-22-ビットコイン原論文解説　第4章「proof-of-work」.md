...
このようProof of Workによってノンスが見つかるとブロックが生成され、また次のブロックを生成するためのハッシュ計算が始まります。ノンスが見つかるスピードは、単純に機械の計算能力で決まります。計算能力があればあるほど、ブロック生成スピードが上がります。

もし攻撃者が取引を改ざんしようとすると、インプットの値が変わる（例えば「Blockchain」が「blockchain」になる）ので、出てくるハッシュ値がかわり、先頭がn個の0を満たすという条件が成り立たなくなります。そのため、攻撃者は再度Proof of Workを行なって、条件を満たすようなノンスを見つけなければなりません。また、ただ該当ブロックのノンスを見つければいいのではなく、ブロックは連鎖していますので、改ざんしたブロックにつながる以降のブロック全てについて正しいノンスを見つけなければなりません。

ただし、攻撃者がいそいそとノンスを計算しているうちにも、他のノードによってブロックは伸び続けています。ビットコインのブロックチェーンは「最も長いチェーンを正しいものとする」という合意があるため、攻撃者は他の善意のノードの計算スピードを上回り、現在のブロックに追いつき、追い越して初めて改ざんに成功します。これを行うためにはノードの総合CPUパワーの過半数を取得しなければならず、大変な労力を必要とします。よくブロックチェーンは改ざん不可能ではないが現実的には困難、と言われるのはこのためです。

### Proof of Workの難易度調整

ブロックが生成されて初めて取引が承認されることになるので、ブロック生成時間はユーザーにとって非常に重要です。ビットコインでは約10分に一つのブロックが生成されるように、Proof of Workの難易度を調整しています。

というのも、ノードが発揮するCPUパワーは、ハードウェアの性能が上昇するため次第に上がります。また、ノード自体の数も増減しうるので、総合的なCPUパワーは可変です。

CPUパワーが可変だと、ブロックの生成スピードも変動することになり、あるブロックは1分で作られたが、今度のブロックは1時間待ってもできない、という事態はユーザーの便益を損ねます。

そのため、Proof of Workの難易度、すなわち先頭の何桁を0で満たせば良いかの桁数は、過去のブロックの生成時間の移動平均を使って都度調整されています。
